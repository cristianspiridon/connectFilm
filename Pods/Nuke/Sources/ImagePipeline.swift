...
public /* final */ class ImagePipeline {
    ...
    public init(configuration: Configuration = Configuration()) {
        ...
        rateLimiter = RateLimiter(queue: queue)

        let isDeduplicationEnabled = configuration.isDeduplicationEnabled
        decompressedImageFetchTasks = TaskPool(isDeduplicationEnabled)
        processedImageFetchTasks = TaskPool(isDeduplicationEnabled)
        originalImageFetchTasks = TaskPool(isDeduplicationEnabled)
        originalImageDataFetchTasks = TaskPool(isDeduplicationEnabled)

        if Configuration.isSignpostLoggingEnabled {
            log = OSLog(subsystem: "com.github.kean.Nuke.ImagePipeline", category: "Image Loading")
        } else {
            log = .disabled
        }
    }
