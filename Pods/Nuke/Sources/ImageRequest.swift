...
public struct ImageRequest: CustomStringConvertible {
    ...
    public init(url: URL,
        ...
        ref = Container(resource: Resource.url(url), processors: processors, priority: priority, options: options)
    ...
    public init(urlRequest: URLRequest,
                ...
                priority: ImageRequest.Priority = .normal,
                options: ImageRequestOptions = .init()) {
        ref = Container(resource: Resource.urlRequest(urlRequest), processors: processors, priority: priority, options: options)
...
extension ImageRequest {
    ...
    func makeLoadKeyForOriginalImage() -> AnyHashable {
        if let loadKey = options.loadKey {
            return loadKey
        }
    ...
    private struct LoadKeyForOriginalImage: Hashable {
        ...
        init(request: ImageRequest) {
            urlString = request.ref.urlString
