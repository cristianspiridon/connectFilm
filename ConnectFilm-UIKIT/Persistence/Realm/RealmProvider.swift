...
class RealmProvider {
    ...
    private static let moviesConfig = Realm.Configuration(
        fileURL: try! Path.inLibrary("movies.realm"),
        schemaVersion: 1,
        deleteRealmIfMigrationNeeded: true,
        objectTypes: [MovieObject.self]
    )

    public static var movies: RealmProvider = {
        RealmProvider(config: moviesConfig)
    }()
...
private let groupId = "group.com.thisisinsomnia.cristians"
